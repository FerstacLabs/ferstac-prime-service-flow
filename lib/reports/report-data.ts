...
export async function buildReportForScope(scope: ReportScope) {
  const [jobs, purchases, workItems, workers, suppliers] = await Promise.all([
    getJobs(),
    getPurchases(),
    getWorkItems(),
    getWorkers(),
...
export async function buildVehicleReportForJob(jobId: string) {
  return loadWorkshopReport("vehicle", parseFilters({ job: jobId }));
}

export function buildPrimeReport(
  scope: ReportScope,
  source: ReportSource,
): PrimeReport {
...
function baseReport(
  ...
  orientation: PrimeReport["orientation"] = "portrait",
): PrimeReport {
  return {
    scope,
    title,
...
function buildOverviewReport({ jobs, purchases, workItems }: ReportSource) {
  ...
  const totals = activeJobs.reduce(
    (acc, job) => {
      ...
      const financial = dbFinancialSummary(
        job.agreed_budget,
        jobPurchases,
        jobWork,
      );
          ...
          rows: activeJobs.map((job) => {
            ...
            return row(
              ...
              presentFields([
                ...
                [
                  "Hədəf təhvil tarixi",
                  formatReportDate(job.target_delivery_date),
                ],
...
function buildPurchasesReport({ jobs, purchases }: ReportSource) {
  const total = purchases.reduce(
    (sum, purchase) => sum + dbPurchaseTotal(purchase),
    0,
  );
  const paid = purchases.reduce(
    (sum, purchase) => sum + purchase.paid_amount,
    0,
  );
  const outstanding = purchases.reduce(
    (sum, purchase) => sum + dbPurchaseOutstanding(purchase),
    0,
  );
  const supplierCount = new Set(
    purchases.map((purchase) => purchase.supplier_id).filter(Boolean),
  ).size;
  return baseReport(
    "purchases",
    "Satınalma hesabatı",
    ...
    [
      {
        ...
        table: {
          ...
          rows: purchases.map((purchase) => {
            const job = jobs.find(
              (item) => item.id === purchase.service_job_id,
            );
            return row(
              purchase.id,
              {
                ...
                supplier:
                  purchase.source_type === "SUPPLIER"
                    ? supplierDisplayName(purchase.suppliers)
                    : "-",
                ...
                buyer: purchase.purchased_by_admin
                  ? "Mən / Administrator"
                  : workerDisplayName(purchase.workers),
              },
              presentFields([
...
function buildWorkersReport({ workers, workItems, jobs }: ReportSource) {
  ...
  const summary = pairs([
    ...
    [
      "Ümumi əmək dəyəri",
      formatReportMoney(
        doneItems.reduce((sum, item) => sum + item.labor_cost, 0),
      ),
    ],
  ]);
  return baseReport(
    ...
    workers.map((worker) => {
      const items = workItems.filter(
        (item) => item.assigned_worker_id === worker.id,
      );
      ...
      return {
        ...
        fields: presentFields([
          [
            "Ad, soyad, ata adı",
            [worker.first_name, worker.last_name, worker.father_name]
              .filter(Boolean)
              .join(" "),
          ],
        ...
        summary: pairs([
          ...
          [
            "Ümumi əmək dəyəri",
            formatReportMoney(
              done.reduce((sum, item) => sum + item.labor_cost, 0),
            ),
          ],
        ]),
        table: {
          ...
          rows: items.map((item) => {
            const job = jobs.find(
              (candidate) => candidate.id === item.service_job_id,
            );
            return row(
              item.id,
              {
...
function buildWorkReport({ jobs, workItems }: ReportSource) {
  const summary = pairs([
    ...
    [
      "Gözləyən",
      String(workItems.filter((item) => item.status === "TODO").length),
    ],
    [
      "İcra olunan",
      String(workItems.filter((item) => item.status === "IN_PROGRESS").length),
    ],
    [
      "Tamamlanan",
      String(workItems.filter((item) => item.status === "DONE").length),
    ],
    [
      "Ləğv edilən",
      String(workItems.filter((item) => item.status === "CANCELLED").length),
    ],
    [
      "Ümumi əmək dəyəri",
      formatReportMoney(
    ...
    [
        ...
        fields: presentFields([
          ...
          [
            ...
            job.insurance_approved_amount === null
              ? ""
              : formatReportMoney(job.insurance_approved_amount),
          ],
        ]),
      },
      {
        ...
        fields: presentFields([
          ...
          [
            ...
            vehicle?.engine_power_hp
              ? `${vehicle.engine_power_hp} hp${vehicle.engine_power_kw ? ` / ${vehicle.engine_power_kw} kW` : ""}`
              : "",
          ],
          [
            "Qeydiyyat şəhadətnaməsi",
            vehicle?.registration_certificate_series_no,
          ],
      ...
      purchasesSection(purchases, jobs),
      {
        ...
        summary: pairs([
          ...
          ["Ümumi xərc", formatReportMoney(financial.totalCost)],
          [
            "Təchizatçılara ödənilmiş",
            formatReportMoney(financial.totalPaidToSuppliers),
          ],
          [
            "Ödənilməmiş borc",
            formatReportMoney(financial.unpaidSupplierAmount),
          ],
          ["Qalan büdcə", formatReportMoney(financial.remainingBudget)],
          [
            "Təxmini mənfəət",
            formatReportMoney(financial.estimatedGrossProfit),
          ],
        ]),
      },
    ],
      ...
      rows: workItems.map((item) =>
        row(
          ...
          presentFields([
            [
              "Servis kartı",
              dash(jobs.find((job) => job.id === item.service_job_id)?.job_no),
            ],
            ["Qeyd", item.notes],
          ]),
        ),
        ...
        row(
          ...
          presentFields([
            [
              "Servis kartı",
              dash(
                jobs.find((job) => job.id === purchase.service_job_id)?.job_no,
              ),
            ],
            [
              "OEM / sənəd",
              [purchase.part_code_oem, purchase.document_no]
                .filter(Boolean)
                .join(" / "),
            ],
            ["Brend/model", purchase.brand_model],
